...
# -------------------------------
# Pollution Reduction Advisor
# -------------------------------
@app.route("/pollution-advice", methods=["POST"])
def pollution_advice():
    data = request.get_json() or {}
    aqi = data.get("aqi")

    if aqi is None:
        return jsonify({"success": False, "error": "AQI required"}), 400
    ...
    if aqi <= 50:
        suggestions = [
            "🌱 Great air quality! Prefer walking or cycling.",
            "🔌 Use renewable energy where possible.",
            "🌳 Plant trees to maintain clean air."
        ]
        ...
        suggestions = [
            "🚗 Try carpooling to reduce traffic pollution.",
            "🏠 Limit outdoor burning activities.",
        ...
        suggestions = [
            ...
            "🏭 Report pollution sources nearby.",
            "🌳 Increase indoor plants for air purification."
        ]
...
def safe_route():
    ...
    if not source or not destination:
        return jsonify({"success": False, "error": "Missing locations"}), 400
    ...
    for r in routes:
        r["score"] = (r["avg_aqi"] * 0.7) + (r["time"] * 0.3)
